...
# https://github.com/hjaurum/DHash/tree/master

"""

The dHash algorithm is a perceptual hashing algorithm and is used for image similarity comparison.

Perceptual hashing algorithms currently include:

aHash: average hash, fast, but low accuracy.
pHash: Perceptual hashing, high accuracy, but slow.
dHash：差异值哈希，速度快，且准确率高。

1. Get the dHash value of the image:
hash = DHash.calculate_hash(image)
2. Calculate the Hamming distance between two images:
hamming_distance = DHash.hamming_distance(image1, image2)
3. Calculate the Hamming distance between two dHash values:
